    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        return http
                ...
                .authorizeHttpRequests(auth -> auth
                        ...
                        .requestMatchers("/health", "/info", "/swagger-ui.html", "/swagger-ui/**", "/api-docs/**",
                                "/h2-console/**", "/actuator/**")
                        .permitAll()
                        ...
                        // Endpoints de Pedido: Apenas CLIENTE pode criar/ver pedidos
                        .requestMatchers("/api/pedidos").hasAuthority("ROLE_CLIENTE")
                        .requestMatchers("/api/pedidos/**").hasAuthority("ROLE_CLIENTE")

                        // Endpoints de Produto: RESTAURANTE gerencia seus produtos, ADMIN tem acesso
                        // total
                        .requestMatchers(HttpMethod.POST, "/api/produtos")
                        .hasAnyAuthority("ROLE_RESTAURANTE", "ROLE_ADMIN")
                        .requestMatchers(HttpMethod.PUT, "/api/produtos/**")
                        .hasAnyAuthority("ROLE_RESTAURANTE", "ROLE_ADMIN")
                        .requestMatchers(HttpMethod.PATCH, "/api/produtos/**")
                        .hasAnyAuthority("ROLE_RESTAURANTE", "ROLE_ADMIN")
                        .requestMatchers(HttpMethod.GET, "/api/produtos/**").authenticated()

                        // Endpoints de Restaurante: ADMIN cadastra/edita, leitura aberta para
                        // autenticados
                        .requestMatchers(HttpMethod.POST, "/api/restaurantes").hasAuthority("ROLE_ADMIN")
                        .requestMatchers(HttpMethod.PUT, "/api/restaurantes/**").hasAuthority("ROLE_ADMIN")
